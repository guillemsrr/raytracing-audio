...
namespace Light
{
    PathTracer::PathTracer(Camera* camera) : RaytracerBase(camera)
    {
    }

    color PathTracer::TraceRay(const Ray& ray) const
    {
        color radiance(0.0f);
        ...
        {
            const HitResult hit = _intersector.ClosestHit(currentRay);
            if (!hit.HasHit())
            {
    ...
    color PathTracer::ShadeSurface(const HitResult& hit, const Ray& ray) const
    {
        const Material& material = *hit.ObjectHit->GetMaterial();
    ...
    color PathTracer::GetSkyColor(const glm::vec3& rayDirection) const
    {
